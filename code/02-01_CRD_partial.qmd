...
```{r crd_dfw}
crd_dfw <- crd_df %>% 
  mutate_at(.vars = c("trt", "rep", "nrate_kgha", "krate_kgha"), .funs = factor) %>% 
  mutate(trt_name = paste0(nrate_kgha, "+", krate_kgha))
...
```{r nk boxplot}
ggplot(data = crd_dfw, aes(y = yield_kgha,
                           x = nrate_kgha,
                           color = nrate_kgha))+
  geom_boxplot()+
  geom_jitter()+
  facet_grid(~krate_kgha, scale= "free")
```
...
# Model fitting
crd_mod <- lm(yield_kgha ~ nrate_kgha * krate_kgha, data = crd_dfw)
...
ggplot(crd_resid, aes(x = .fitted, y = .studresid))+
  geom_point(shape = 21,
             ...
             size = 3, 
             alpha = 0.3)+
  geom_hline(yintercept = c(0,3, -3), linetype = "dashed", color = "red")+
  geom_smooth()
...
library(olsrr)
library(lmtest)
ols_plot_resid_lev(crd_mod)

resettest(crd_mod)

shapiro.test(crd_mod$residuals)

ncvTest(crd_mod, ~nrate_kgha*krate_kgha) # coming from car package
```
...
crd_means_all <- emmeans(crd_mod, ~nrate_kgha:krate_kgha)
...
```{r crd final plot}
ggplot()+
  geom_boxplot(data = crd_dfw, aes(x = trt_name, y = yield_kgha))+
  geom_label(data = crd_cld_selected, aes(x = trt_name, label = letter, y = emmean))
